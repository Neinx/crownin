...
function playlistClick(mouseEvent) {
	...
	if (hasClassName(eventTarget, "jbuy") || hasClassName(eventTarget.parentNode, "jbuy") || hasClassName(eventTarget.parentNode.parentNode, "jbuy")) {
		var songInfo = eventTR.id.split('|');
		if (lastBuyMenu == songInfo[0] && document.getElementById("jbuy_menu").style.display == 'block') {
			$('#jbuy_menu').slideUp();
			removeClassName(lastBuyButton, "open");
		} else {
			document.getElementById('jbuy_menu').style.top = (eventTR.offsetTop + 16) + 'px';
			document.getElementById('jbuy_menu').style.display = 'none';
			if (lastBuyButton) removeClassName(lastBuyButton, "open");
			addClassName(document.getElementById('jbuy_'+songInfo[0]), "open");
			var G_checked = (cart_items.indexOf('G'+songInfo[0]) != -1) ? 1 : 0;
			var P_checked = (cart_items.indexOf('P'+songInfo[0]) != -1) ? 1 : 0;
			var D_checked = (cart_items.indexOf('D'+songInfo[0]) != -1) ? 1 : 0;
			var buy_content = '';
			if (songInfo[5] > 0) buy_content += '<a href="javascript:void(0)" onclick="addToCart(\'G' + songInfo[0] + '\',19.95,\'' + songInfo[1] + ' (Lease, MP3 file)\')" class="jbuy_link' + (G_checked ? ' checked' : '') + '" id="G' + songInfo[0] + '_2">Buy Lease (MP3 file) $19.95</a><br />';
			if (songInfo[6] > 0) buy_content += '<a href="javascript:void(0)" onclick="addToCart(\'P' + songInfo[0] + '\',39.95,\'' + songInfo[1] + ' (Lease, WAV file)\')" class="jbuy_link' + (P_checked ? ' checked' : '') + '" id="P' + songInfo[0] + '_2">Buy Lease (WAV file) $39.95</a><br />';
			if (songInfo[7] > 0) buy_content += '<a href="javascript:void(0)" onclick="addToCart(\'D' + songInfo[0] + '\',79.95,\'' + songInfo[1] + ' (Lease, Tracked Out)\')" class="jbuy_link' + (D_checked ? ' checked' : '') + '" id="D' + songInfo[0] + '_2">Buy Lease (Tracked Out) $79.95</a><br />';
			buy_content += '<div class="jbuy_licensing_info"><a href="javascript:void(0)" onclick="showSidebar(1)" class="jbuy_link">Show Licensing Info</a></div>';
			document.getElementById('jbuy_menu').innerHTML = buy_content;
			$('#jbuy_menu').slideDown();
			if (eventTR.offsetTop - eventTR.parentNode.parentNode.parentNode.scrollTop + 100 > eventTR.parentNode.parentNode.parentNode.offsetHeight) {
				$('#jplaylist_1').animate({scrollTop : eventTR.offsetTop - eventTR.parentNode.parentNode.parentNode.offsetHeight + 100});
			}
			lastBuyMenu = songInfo[0];
			lastBuyButton = document.getElementById('jbuy_'+songInfo[0]);
		}
...
function playClick() {
	if (!hasClassName(document.getElementById('jplay'), 'playing')) {
		...
		document.getElementById('song_click').style.cursor = "pointer";
		callToActionscript('jeejuhplayer', 'play.'+currentHash);
	} else {
		...
		currentInit = '';
		callToActionscript('jeejuhplayer', 'pause');
	}
}
...
function playlistNav(nav) {
	if (nav == "complete" && loop == 1) {
		callToActionscript('jeejuhplayer', 'playfromtop.'+currentHash);
	} else if (nav == "complete" && hasClassName(document.getElementById('jprev'), 'disabled')) {
		document.getElementById('song_time').innerHTML = "0:00";
		document.getElementById('song_blue').style.width = "0";
		removeClassName(document.getElementById('jplay'), 'playing');
		removeClassName(document.getElementById('speaker_'+currentSongID), 'playing');
...
function playEventTR(playfromtop) {
	var songInfo = eventTR.id.split('|');
	if (songInfo[11] == 'youtube') {
		if (document.getElementById('jvideo').style.display != 'block') {
			callToActionscript('jeejuhplayer', 'pause');
			$('#jvideo').slideDown().delay(400).hide(0).show(0);
			document.getElementById('jplay').style.display = 'none';
			document.getElementById('jprev').style.marginLeft = '39px';
			document.getElementById('song_player').style.display = 'none';
			removeClassName(document.getElementById('song_thumb'), 'open');
			document.getElementById('song_thumb').innerHTML = '';
			document.getElementById('song_by').innerHTML = '<br />From: <a href="http://www.youtube.com/user/'+songInfo[2]+'" target="_blank">'+songInfo[2]+'</a> &nbsp;<span class="pipe">|</span> '+songInfo[12];
			addClassName(document.getElementById('song_info'), 'open');
		}
		document.getElementById('jvideo').innerHTML = '<div class="jvideo_close"><a href="javascript:void(0)" onclick="closeVideo()">Close</a></div><iframe width="854" height="510" src="http://www.youtube.com/embed/' + songInfo[9] + '?theme=dark&autoplay=1" frameborder="0" allowfullscreen></iframe>';
	} else {
		if (document.getElementById('jvideo').style.display == 'block') {
			$('#jvideo').slideUp();
			var browserScroll;
			if (window.pageYOffset) browserScroll = window.pageYOffset;
			else if (document.documentElement) browserScroll = document.documentElement.scrollTop;
			else if (document.body) browserScroll = document.body.scrollTop;
			$("html:not(:animated),body:not(:animated)").animate({scrollTop: Math.max(0, browserScroll - 510)});
		}
		document.getElementById('jvideo').innerHTML = '';
		document.getElementById('jplay').style.display = 'block';
		document.getElementById('jprev').style.marginLeft = '19px';
		document.getElementById('song_player').style.display = 'block';
		removeClassName(document.getElementById('song_info'), 'open');
		if (songInfo[11]) {
			document.getElementById('song_thumb').innerHTML = '<a href="'+songInfo[11]+'" target="_blank"><img src="/thumbs/'+(songInfo[12] ? songInfo[12] : 'blank.gif')+'" width="47" height="47" /></a>';
			document.getElementById('song_by').innerHTML = ' &nbsp;by <a href="'+songInfo[11]+'" target="_blank">'+songInfo[2]+'</a>';
		} else {
			document.getElementById('song_thumb').innerHTML = '<img src="/thumbs/'+(songInfo[12] ? songInfo[12] : 'blank.gif')+'" width="47" height="47" />';
			document.getElementById('song_by').innerHTML = ' &nbsp;by <b>'+songInfo[2]+'</b>';
		}
		addClassName(document.getElementById('song_thumb'), 'open');
	}
	document.getElementById('song_title').innerHTML = songInfo[1];
	var share_content = '<table class="jshare_table"><tr>';
	var link_left = (songInfo.length > 12 && songInfo[11] != 'youtube') ? 282 : 241;
	if (songInfo[10]) link_left += 63;
	if (songInfo[10]) share_content += '<td><a href="' + songInfo[10] + '">Download</a> &nbsp;<span class="pipe">|</span> Share:</td>';
	else share_content += '<td>Share:</td>';
	share_content += '<td><a href="http://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Fwww.jeejuh.com%2F' + songInfo[8] + '" title="Share on Facebook" target="_blank"><div class="jshare_facebook"></div></a> <a href="http://twitter.com/share?url=http%3A%2F%2Fwww.jeejuh.com%2F' + songInfo[8] + '&amp;text=' + songInfo[1] + ' - ' + ((songInfo.length < 12) ? 'Beat ' : '') + 'by ' + songInfo[2] + ' @JeeJuh" title="Share on Twitter" target="_blank"><div class="jshare_twitter"></div></a> <a href="javascript:void(0)" title="Share URL" onclick="showLink(' + link_left + ', \'' + (songInfo[11] == 'youtube' ? 'video' : (songInfo.length > 12 ? 'song' : 'beat')) + '\', \'' + songInfo[8] + '\')"><div class="jshare_link"></div></a></td>';
	share_content += '<td>&nbsp; &nbsp;<span class="pipe">|</span> <a href="javascript:void(0)" onclick="showSidebar(2)"><span id="song_comments_count">Comments</span></a></td>';
	share_content += '</tr></table>';
	document.getElementById('jshare').innerHTML = share_content;
	...
	var likes_content = '<div class="likes_bar" id="likes_bar">';
	if (songInfo[3] == 0 && songInfo[4] == 0) {
		likes_content += '<div class="white_bar"></div>';
	} else if (songInfo[3] > 0 && songInfo[4] > 0) {
		var likespixels = Math.ceil(98 * Number(songInfo[3]) / (Number(songInfo[3]) + Number(songInfo[4])));
		likes_content += '<div class="green_bar" style="width:' + likespixels + 'px"></div><div class="red_bar" style="width:' + (98 - likespixels) + 'px"></div>';
	} else if (songInfo[3] > 0) {
		likes_content += '<div class="green_bar" style="width:100px"></div>';
	} else {
		likes_content += '<div class="red_bar" style="width:100px"></div>';
	}
	...
	document.getElementById('song_likes').innerHTML = likes_content;
	var buy_content = '';
	if (document.getElementById('beatused_'+songInfo[0]).innerHTML) buy_content += '&nbsp; &nbsp; &nbsp; &nbsp; Beat: ' + document.getElementById('beatused_'+songInfo[0]).innerHTML + '<br />';
	buy_content += '&nbsp; &nbsp; &nbsp; &nbsp; Producer: ' + document.getElementById('producer_'+songInfo[0]).innerHTML;
	document.getElementById('song_buy').innerHTML = buy_content;
	removeClassName(document.getElementById('speaker_'+currentSongID), 'playing');
	...
	$.getJSON('http://graph.facebook.com/?ids=http://www.jeejuh.com/' + songInfo[8] + '&callback=?',function(data){
		for(object in data){
			num = data[object].comments ? data[object].comments : 0;
			document.getElementById('song_comments_count').innerHTML = num + ' Comment' + (num == 1 ? '' : 's');
        }
	});
	if (songInfo[11] != 'youtube') {
		document.getElementById('song_time').innerHTML = "0:00";
		if (currentTR != eventTR) document.getElementById('song_loading').style.width = "0";
		document.getElementById('song_blue').style.width = "0";
		document.getElementById('song_click').style.cursor = "pointer";
		callToActionscript('jeejuhplayer', (playfromtop ? 'playfromtop' : 'play') + '.' + songInfo[9]);
	}
	removeClassName(currentTR, "current");
	addClassName(eventTR, "current");
	currentTR = eventTR;
	scrollToSong();
	if (currentCommentsSlug != currentSlug && hasClassName(document.getElementById('jcomments_link'), 'open')) {
...
function libraryClick(lib) {
	...
	document.getElementById('jsearch_close_'+currentLibrary).style.display = "none";
	if ((lib == 2 && document.getElementById('jsearch_'+lib).value != 'Search Songs') || (lib == 3 && document.getElementById('jsearch_'+lib).value != 'Search...')) document.getElementById('jsearch_close_'+lib).style.display = "block";
...
function sortPlaylist(playlist,headcol,col,num,desc,fragmented) {
...
function searchPlaylist(playlist, query) {
	...
	if (query != lastQuery) {
		...
		var totalseconds = 0;
		for (var i=0; i<tbody.rows.length; i++) {
			if ((query == '' || tbody.rows[i].cells[0].id.indexOf(query) > -1)) {
		...
		document.getElementById('jsearch_close_'+currentLibrary).style.display = (query == '') ? 'none' : 'block';
		document.getElementById('jtotals_'+currentLibrary).innerHTML = found + (currentLibrary == 1 ? ' beat' : (currentLibrary == 2 ? ' song' : ' video')) + (found == 1 ? ', ' : 's, ') + secondsToDecimal(totalseconds);
		lastQuery = query;
	}
}
...
function showSidebar(page) {
	if (!hasClassName(document.getElementById('jcomments_link'), 'open')) {
		$("#jplaylist_container").animate({width:"396px"});
		$("#jheader_2").animate({width:"396px"});
		$("#jheader_3").animate({width:"396px"});
		...
		$("#jheader_2").animate({width:"825px"});
		$("#jheader_3").animate({width:"825px"});
...
function syncScroll(playlist, header) {
	document.getElementById('jheader_'+header).scrollLeft = playlist.scrollLeft;
}

function trackClick(obj, e) {
	if (document.getElementById('song_loading').style.width) {
		if (!e) var e = window.event;
		clickX = e.clientX - obj.offsetLeft;
		if (clickX <= 5) clickX = 0;
		clickX = Math.min(clickX, 199, parseInt(document.getElementById('song_loading').style.width) - 1);
		document.getElementById('song_blue').style.width = Math.max(0, clickX - 1) + "px";
		callToActionscript('jeejuhplayer', 'track.'+clickX);
	}
}
...
function sendToJavaScript(val) {
	var call = val.split('.');
	if (call[0] == 'time') {
		document.getElementById('song_time').innerHTML = call[2];
	} else if (call[0] == 'loading') {
		document.getElementById('song_loading').style.width = call[2] + "px";
	} else if (call[0] == 'blue') {
		document.getElementById('song_blue').style.width = call[2] + "px";
	} else if (call[0] == 'complete') {
		playlistNav('complete');
	} else if (call[0] == 'loaded' && currentInit && currentLibrary != 3) {
		callToActionscript('jeejuhplayer', 'play.'+currentInit);
	}
}
...
function likeClick(liked) {
	if (!hasClassName(document.getElementById('like_button'), 'clicked') && !hasClassName(document.getElementById('dislike_button'), 'clicked')) {
		...
		if (likes > 0 && dislikes > 0) {
			...
			document.getElementById('likes_bar').innerHTML = '<div class="green_bar" style="width:' + likespixels + 'px"></div><div class="red_bar" style="width:' + (98 - likespixels) + 'px"></div>';
			var mini_likespixels = Math.ceil(48 * likes / (likes + dislikes));
			document.getElementById('mini_likes_bar_'+currentSongID).innerHTML = '<div class="green_bar white_right" style="width:' + mini_likespixels + 'px"></div><div class="red_bar" style="width:' + (48 - mini_likespixels) + 'px"></div>';
		} else if (likes > 0) {
			document.getElementById('likes_bar').innerHTML = '<div class="green_bar" style="width:100px"></div>';
			document.getElementById('mini_likes_bar_'+currentSongID).innerHTML = '<div class="green_bar" style="width:50px"></div>';
		} else {
			document.getElementById('likes_bar').innerHTML = '<div class="red_bar" style="width:100px"></div>';
			document.getElementById('mini_likes_bar_'+currentSongID).innerHTML = '<div class="red_bar" style="width:50px"></div>';
		}
...
function OnMouseDown(e) {
	...
	if (target.tagName == 'HTML' || target.tagName == 'BODY' || (!hasClassName(target, "jbuy") && !hasClassName(target.parentNode, "jbuy") && !hasClassName(target.parentNode.parentNode, "jbuy") && !hasClassName(target, "jbuy_menu") && !hasClassName(target, "jbuy_licensing_info") && !hasClassName(target, "jbuy_link"))) {
		$('#jbuy_menu').slideUp();
		if (lastBuyButton) removeClassName(lastBuyButton, "open");
	}
	...
	if ((e.button == 1 && window.event != null || e.button == 0) && (target.className == 'jvol' || target.parentNode.className == 'jvol')) {
		target = document.getElementById('jvol_knob');
		target.style.backgroundPosition = '-348px -255px';
		document.body.style.cursor = 'pointer';
		_startX = e.clientX;
		_offsetX = e.x-target.offsetLeft;
...
function OnMouseMove(e) {
	...
	newVol = Math.min(Math.max(0, currentVol + (_offsetX - 6 + e.clientX - _startX)), 68);
	passVol = newVol;
	callToActionscript('jeejuhplayer', 'vol.'+newVol);
	document.getElementById('jvol_knob').style.left = (newVol + 11) + 'px';
