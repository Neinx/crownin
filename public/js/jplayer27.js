...
function sortPlaylist(playlist,headcol,col,num,desc,fragmented,always) {
	...
	if (headcol) {
        ...
        if (col == 4 && document.getElementById('jsortSortBy').selectedIndex != 6) document.getElementById('jsortSortBy').selectedIndex = 6;
				if (col == 5 && document.getElementById('jsortSortBy').selectedIndex != 0) document.getElementById('jsortSortBy').selectedIndex = 0;
			}
		}
	} else {
		theadcells = document.getElementById("thead"+playlist).rows[0].cells;
...
function updateSorting(type) {
    ...
    if (jsortSortBy == "Sounds Like") sortPlaylist(1,5,5);
		if (jsortSortBy == "Price") sortPlaylist(1,6,6,1);
	} else {
		if (type == 4) {
			...
			xmlhttp.onreadystatechange = function() {
				if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
					if (xmlhttp.responseText == 1) {
						...
						document.getElementById('offer_form_button').disabled = false;
						alert("We were unable to send your offer due to a techinical error. Please email us directly at beats@jeejuh.com");
					}
				}
			}
 ...
 */
 ...
 $(document).ready(function() {
	$("a.iframe").fancybox({
		onStart: function(){
			if (hasClassName(document.getElementById('jplay'), 'playing')) playClick();
		}
	});
	$("a.about_us").fancybox();
});
